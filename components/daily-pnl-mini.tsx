...
export function DailyPnlMini({ data }: { data: DailyPnlPoint[] }) {
  ...
  return (
    <Card className="flex-1 p-4">
      <h2 className="mb-2 text-sm font-medium text-muted-foreground">Daily net cumulative P&L</h2>
      ...
      <div className="mt-3 border-t pt-3">
        <h3 className="mb-1 text-xs font-medium text-muted-foreground">Daily P&L</h3>
        <div className="h-20 w-full">
          {daily.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={daily} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
                <Tooltip
                  ...
                  labelStyle={{ color: "var(--muted-foreground)" }}
                  formatter={(value) => [formatCurrency(Number(value)), "Day P&L"]}
                />
            ...
            <div className="flex h-full items-center justify-center text-xs text-muted-foreground">No closed days yet.</div>
          )}
        </div>
      </div>
    </Card>
