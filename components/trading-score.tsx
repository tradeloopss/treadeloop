...
export function TradingScore({ overall, axes }: { overall: number; axes: ScoreAxis[] }) {
  ...
  const data = axes.map((axis) => ({ ...axis, label: t(axis.label) }))

  return (
    <Card className="p-4">
      <h2 className="mb-2 text-sm font-medium text-muted-foreground">{t("TradeLoop Score")}</h2>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={data} outerRadius="70%">
            ...
            <Tooltip
              ...
              formatter={(value) => [`${Number(value).toFixed(0)} / 100`, t("Score")]}
            />
      ...
      <div className="mt-2 flex items-center justify-between gap-3 border-t pt-2">
        <div>
          <p className="text-[10px] text-muted-foreground">{t("Your score")}</p>
